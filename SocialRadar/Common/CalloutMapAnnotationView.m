...
- (id) initWithAnnotation:(id <MKAnnotation>)annotation reuseIdentifier:(NSString *)reuseIdentifier
{
	if (self = [super initWithAnnotation:annotation reuseIdentifier:reuseIdentifier])
    {
		...
		//self.offsetFromParent = CGPointMake(0, 0); //this works for MKPinAnnotationView
		self.enabled = NO;
		self.backgroundColor = [UIColor clearColor];
       // self.parentAnnotationView.frame.size = CGSizeMake(33, 46);
        
         self.parentAnnotationView.image=[UIImage imageWithContentsOfFile: [[NSBundle mainBundle] pathForResource: @"blog" ofType: @"png"]];
...
//	self.offsetFromParent.y;
    ...
    frame.size = CGSizeMake(117.5, 74);
	self.frame = frame;
}
...
- (void)prepareOffset
{
    ...
    self.centerOffset = CGPointMake(-25, -55);
}
...
- (void)drawRect:(CGRect)rect {
	...
	CGContextSetShadowWithColor(context, CGSizeMake (0, self.yShadowOffset), 6, [UIColor clearColor].CGColor);
...
{
    //appDelegate.isMyLocation = YES;
    
}

- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    [self.mapView selectAnnotation: self.parentAnnotationView.annotation animated: YES];
}
